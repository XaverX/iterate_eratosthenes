...
def DoAllTheStuff():
    ...
    n = np.int64(2)  # n = 5
    P = np.array([2], dtype='int64')  # P = [2, 3, 5]
    # J = np.array([6, 4, 2, 4, 2, 4, 6, 2], dtype='int64')
    J = np.array([2], dtype='int8')
    pp = np.int64(1)
    ppp = np.int64(1)
    ll = np.int64(1)
    lll = np.int64(1)
    ...
    while n < MAXPRIME:
        ...
        print('*', end='')
        J = np.ravel(
                np.array(np.arange(n) * pp, dtype='int64')[:, np.newaxis] + J)
        ###
        print('*', end='')
        J = np.compress(np.in1d(J, S, assume_unique=True, invert=True), J)
        ###

        # formerly K now in J
        print('\b\bK _', J.shape, J.size, J.itemsize,
              J[:min(6, J.size//2)], '..', J[-min(6, J.size//2):])

        J = np.array(np.diff(J, prepend=1), dtype='int8')
        print('J >', J.shape, J.size, J.itemsize,
              J[:min(5, J.size//2)], '..', J[-min(6, J.size//2):])
